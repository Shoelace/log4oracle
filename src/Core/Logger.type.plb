...
AS
	...
	MEMBER FUNCTION isTraceEnabled RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_TRACE,NULL); END;
	MEMBER FUNCTION isDebugEnabled RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_DEBUG,NULL); END;
	MEMBER FUNCTION isInfoEnabled  RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_INFO ,NULL); END;
	MEMBER FUNCTION isWarnEnabled  RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_WARN ,NULL); END;
	MEMBER FUNCTION isErrorEnabled RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_ERROR,NULL); END;
	MEMBER FUNCTION isFatalEnabled RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_FATAL,NULL); END;

	MEMBER FUNCTION isTraceEnabled(marker Marker) RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_TRACE,marker); END;
	MEMBER FUNCTION isDebugEnabled(marker Marker) RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_DEBUG,marker); END;
	MEMBER FUNCTION isInfoEnabled (marker Marker) RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_INFO ,marker); END;
	MEMBER FUNCTION isWarnEnabled (marker Marker) RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_WARN ,marker); END;
	MEMBER FUNCTION isErrorEnabled(marker Marker) RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_ERROR,marker); END;
	MEMBER FUNCTION isFatalEnabled(marker Marker) RETURN BOOLEAN IS BEGIN RETURN isEnabled(Logger_impl.ll_FATAL,marker); END;
