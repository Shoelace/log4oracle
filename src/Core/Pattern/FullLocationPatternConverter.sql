...
create or replace 
type  FullLocationPatternConverter 
under    LogEventPatternConverter 
(
	--static function Convert(event LogEvent, value varchar2) return varchar2,
	
	constructor function FullLocationPatternConverter(options varchar2) return self as result,
...
create or replace 
type body FullLocationPatternConverter as
  ...
  BEGIN
	...
	constructor function FullLocationPatternConverter(options varchar2) return self as result is
	begin
	...
	overriding member function Format(event LogEvent) return varchar2 is
		element StackTraceElement := event.getSource();
	begin
